...
block or not part of the module's own storage: the exact shape every
`$display`-bearing always block in the corpus takes, since the Bluespec
runtime's error-reporting block writes only a block-local flag register
declared right next to the display call. Third, a `Delay` node or a
`TaskCall`, neither of which occurs anywhere in the corpus, kept here so
the predicate is complete rather than corpus-shaped.

`partition_always_blocks(module_ir)` applies the always-block shape above
to `module_ir.always_blocks`, returning the surviving blocks and the
dropped blocks separately, so the emitter takes only the survivors and a
test can assert on both sides at once. `module_ir.initials` is a distinct
list the parser already builds independently (every `Initial` node is
...
def partition_always_blocks(module_ir) -> tuple[list, list]:
    ...
    against the set of every signal name the module declares at module
    scope (`module_ir.signals`); a target declared only inside the block
    itself is never part of that set, so it can never make a block survive
    on its own.
    """
